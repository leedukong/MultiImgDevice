...
public class PasswordManager {
    ...
    public void passwordDialog(AppCompatActivity activity) {
        ...
        AlertDialog.Builder passwordDialogBuilder = new AlertDialog.Builder(context).setTitle("패스워드를 입력하세요.").setView(passwordLayout).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                String userEnteredPassword = pw.getText().toString();
                if (userEnteredPassword.equals(managerPassword)) {
                    Intent intent = new Intent(context, ManagerSettings.class);
