...
public class DBManager {
    ...
    public boolean updateColumnTodecreaseCount(long id) {
        ContentValues values = new ContentValues();
        values.put(DataBase.CreateProductDessertDB.CURRENT_COUNT, DataLoader.loadCurrentStockById(this, id) - 1);
        if (!DBManager.mDB.isOpen()) {
            open();
            create();
        }
    ...
    public boolean updateColumnToRechargeCount(long id) {
        ContentValues values = new ContentValues();
        values.put(DataBase.CreateProductDessertDB.CURRENT_COUNT, DataLoader.loadProductById(this, id).total_count);
        if (!DBManager.mDB.isOpen()) {
            open();
            create();
        }
